...
public class VmAllocationPolicyCombinedMostFullFirst extends VmAllocationPolicy implements PowerUtilizationMaxHostInterface {
	...
	public boolean allocateHostForVm(Vm vm) {
		...
		System.out.println("pes:"+requiredPes);
		double requiredMips = vm.getCurrentRequestedTotalMips();
		long requiredBw = vm.getCurrentRequestedBw();
		//long reuiredRam = vm.getCurrentRequestedRam();

		totalRequestedMips += vm.getCurrentRequestedTotalMips();
		totalRequestedMemory += vm.getCurrentRequestedRam();
		System.out.println("MIPPPPPSSSSS:"+totalRequestedMips);
		System.out.println("RAAAAAM:"+totalRequestedMemory);
		...
		//for (Integer freePes : getFreePes()) {
			...
			System.out.println("mipsFreePercent:" +mipsFreePercent);
			System.out.println("bwFreePercent:" +bwFreePercent);
			double cpuUtilization = totalRequestedMips / this.hostTotalMips;
			System.out.println("CPUUU:"+cpuUtilization);
			//double memoryUtilization = totalRequestedMemory / this.hostTotalRam();
		...
		//for(int tries = 0; result == false && tries < numHosts; tries++) {// we still trying until we find a host or until we try all of them
				...
				//initialization(freePesTmp);
				initialization(freeResources);
				System.out.println("Passei por aqui");
				try {
					exibirPopulacao(population2);
					System.out.println("---");
					//					Thread t = new Thread();			
					//					t.sleep(5000);
	...
	public void exibirPopulacao(String [][] population1){
...
//v_posString =  v_posString.replace(",",".");
	...
	public void generateRoulette(String [] roulette){
		...
		while (i<popSize){
			...
			//energia = Float.parseFloat(population1[i][fitnessLocation]);
			energia = Double.parseDouble(population1[i][fitnessLocation]);
			soma += energia;
...
//int v_CurrentPosX = Math.round(Float.parseFloat(v_posString));
	...
	/*public void generateRoulette(String [] roulette){
		//sum of the fitness of all the chromosomes
		float cumulativeFitnesses=0;
		float strength = 0;
	
		int i=0;
		while (i<popSize){

			strength = Float.parseFloat(population1[i][fitnessLocation]);
			cumulativeFitnesses = cumulativeFitnesses + strength;

			i++;
		}//fim while

		float lastProbabaility=0;
		...
		while (i<popSize){
			...
			strength = Float.parseFloat(population1[i][fitnessLocation]);
			currentProbability = lastProbabaility + strength/cumulativeFitnesses;
			roulette[i] = currentProbability + "";
			lastProbabaility = currentProbability;
			i++;
	...
	public int chromoRoulette(String [] roulette, double p){
		...
		while (i<roulette.length && !isFound){
			...
			if (p>=anterior && p <= Double.parseDouble(roulette[i])){
				bestIndividual = i;
				isFound=true;
			} else
				anterior = Double.parseDouble(roulette[i]);
...
//--------------------------------SORT-----------------------------------
	public void sortPopulation() {
		for (int pass=1; pass<popSize; pass++) //passagens
			for (int i=0; i<popSize-1; i++) //uma passagem		
	...
	public void sortIndividual(int chromoIndex,List<Double> freeResources){
		...
		int host1=1;
		int host2=1;
		for (int pass=1; pass<chromoLength-1; pass++) //passagens (nao conta o campo de fitness)
			for (int i=0; i<chromoLength-2; i++){//uma passagem (nao conta o campo de fitness)
				host1=Integer.parseInt(population2[chromoIndex][i])-1; //-1 pq a lista de hosts comeca em 0
				host2=Integer.parseInt(population2[chromoIndex][i+1])-1; //-1 pq a lista de hosts comeca em 0
				//Ordena os cromossomos com base nas cpus disponiveis
